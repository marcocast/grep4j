...
public class WhenGreppingSingleProfile {
	...
	GrepResults results = grep(naturalExpression(STRING_TO_SEARCH), on(localProfile()));
	assertThat(results, containsExpression("ERROR"));
    }

    public void only2ErrorStringsMustNotBeFoundOnTheResult() {
	GrepResults results = grep(naturalExpression("ERROR"), on(localProfile()));
	assertThat(results, doesNotContainExpression("3"));
    }

    public void gzStringsShouldBeFoundOnTheResult() {
	GrepResults results = grep(naturalExpression("ERROR"), on(localProfileWithWildecard("*")));
	assertThat(results, containsExpression("GZ"));
    }

    public void fineStringAppears3Times() {
	assertThat(whenCalling(grep(naturalExpression("fine"), on(localProfile()))).totalLines(), is(5));
    }

    public void errorStringAppears2Times() {
	assertThat(executing(grep(naturalExpression("ERROR"), on(localProfile()))).totalLines(), is(2));
    }

    public void error33StringneverAppears() {
	assertThat(executing(grep(naturalExpression("ERROR33"), on(localProfile()))).totalLines(), is(0));
    }

    public void errorMultipleTokenStringStringAppearsOneTime() {
	assertThat(executing(grep(naturalExpression("has been updated"), on(localProfile()))).totalLines(), is(1));
    }

    public void errorStringWithRegExCaracthersAppearsOneTime() {
	assertThat(executing(grep(naturalExpression("Marco(id=12345)"), on(localProfile()))).totalLines(), is(1));
    }
