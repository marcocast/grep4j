...
public class WhenGreppingPizzaFileWithOptions {
	...
	public void ignoreCaseTest() {
		GrepResults results = grep(regularLanguage("PIZZA"), on(pizzaProfile()), ignoreCase());
		assertThat(results.totalLines(), is(5));
	}

	public void extraLinesAfterTest() {
		GrepResults results = grep(regularLanguage("Continuate ad impastare"), on(pizzaProfile()), extraLinesAfter(3));
		assertThat(results.totalLines(), is(3));
	}

	public void extraLinesBeforeAndAfetrTest() {
		GrepResults results = grep(regularLanguage("Coprite la ciotola"), on(pizzaProfile()), extraLinesBeforeAndAfter(1));
		assertThat(results.totalLines(), is(3));
	}

	public void invertMatchTest() {
		GrepResults results = grep(regularLanguage("pizza"), on(pizzaProfile()), invertMatch());
		assertThat(results.totalLines(), is(11));
	}

	public void invertMatchAndWithFileNameTest() {
		GrepResults results = grep(regularLanguage("PIZZA"), on(pizzaProfile()), ignoreCase(), withFileName());
		assertThat(results.totalLines(), is(5));
	}

	public void onlyMatchingTest() {
		GrepResults results = grep(regularLanguage("pizza"), on(pizzaProfile()), onlyMatching());
		assertThat(results.totalLines(), is(5));
	}

	public void countMatchesTest() {
		GrepResults results = grep(regularLanguage("pizza"), on(pizzaProfile()), countMatches());
