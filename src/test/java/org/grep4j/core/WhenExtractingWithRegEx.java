...
public class WhenExtractingWithRegEx {
	...
	for (GrepResult result : results) {
	    if (result.getFileName().endsWith("gz")) {
		assertThat(StringUtils.contains(result.filterByRE("OR").filterByRE("2").getText(), "GZ ERROR 1"),
			is(false));
		assertThat(StringUtils.contains(result.filterByRE("OR").filterByRE("1").getText(), "GZ ERROR 2"),
			is(false));
	    } else {
		assertThat(
			StringUtils.contains(result.filterByRE("fine").filterByRE("extract").filterByRE("(.*)ub(.*)")
				.getText(), "fine double extract"), is(true));
	    }
	}
    }
